...
function identityCard(name, profile, user) {
  ...
  return el("section", { class: PANEL + " overflow-hidden p-0" },
    ...
    el("div", { class: "px-5 pb-5" },
      el("div", { class: "flex items-end justify-between gap-4 -mt-8 flex-wrap" },
        el("div", { class: "flex items-end gap-4 min-w-0" },
          el("div", { class: `w-24 h-24 rounded-full bg-gradient-to-br ${accent.grad} border-4 border-slate-900 flex items-center justify-center text-3xl font-extrabold text-white shrink-0` }, initials || "?"),
          el("div", { class: "pb-1 min-w-0" },
            el("h1", { class: "text-xl font-bold leading-tight truncate" }, name),
